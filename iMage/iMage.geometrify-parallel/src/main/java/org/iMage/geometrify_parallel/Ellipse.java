...
public class Ellipse extends AbstractPrimitive {
	 ...
	 * @param q
	 *            a point determining the horizontal (x-coordinate) and vertical
	 *            (y-coordinate) radius of the ellipse
	 */
	public Ellipse(Point p, Point q) {
		...
		setBoundingBox(new BoundingBox(new Point(p.x - width, p.y - height), new Point(p.x + width, p.y + height)));
	}
